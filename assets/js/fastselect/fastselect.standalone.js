(function(factory) {
    ...
    $.extend(Fastsearch.prototype, {
        ...
        generateItem: function(item) {
			...
			var currop_string = html.toLowerCase();
				
			$('.fstChoiceItem').each(function(){
					
				var added_string = $(this).attr('data-value').toLowerCase();
				
				if(added_string === currop_string || parseInt(item.approved) == 0){
					$tag.addClass('fstSelected');
				}

				if(parseInt(item.approved) == 0){
					$tag.addClass('fstBlocked');
				}	
			});
			
			$('.fstResultItem').each(function(){
				
				var curr_string = $(this).text().toLowerCase();
				
				if(curr_string === currop_string || parseInt(item.approved) == 0){
					
					if($tag.hasClass('fstSelected')){
						// already added
					}else{
						$tag.addClass('fstSelected');
					}
...
(function(root, factory) {
    ...
    $.extend(Fastselect.prototype, {
        ...
        addChoiceItem: function(optionModel, status) {
			
			var addon_string = optionModel.text.toLowerCase();
			addon_string = addon_string.replace('Add','');
			addon_string = $.trim(addon_string);
			...
			$('.fstResultItem').each(function(){
				...
				if(addon_string === curr_string){
					
					if($(this).hasClass('fstSelected')){
			...
			if(parseInt(present) == 1){
				if(parseInt(add_type) == 1){
					$('.msg_error_product').show();
					setTimeout(function(){ $('.msg_error_product').fadeOut(); }, 6000);
				}
				
				if(parseInt(add_type) == 2){
					$('.msg_error_service').show();
					setTimeout(function(){ $('.msg_error_service').fadeOut(); }, 6000);
				}
			}	
			...
			if(parseInt(present) == 0){
				...
				if(status == 0){
					...
					$('<div style="padding-left: 3px;" data-text="' + optionModel.text.charAt(0).toUpperCase() + optionModel.text.slice(1) + '" data-value="' + optionModel.value.charAt(0).toUpperCase() + optionModel.value.slice(1) + '" class="' + this.options.choiceItemClass + '">' +
						$('<div>').html(optionModel.text.charAt(0).toUpperCase() + optionModel.text.slice(1)).text() +
						'</div>'
				...
				if(status == 1){
					...
					$('<div data-text="' + optionModel.text.charAt(0).toUpperCase() + optionModel.text.slice(1) + '" data-value="' + optionModel.value.charAt(0).toUpperCase() + optionModel.value.slice(1) + '" class="' + this.options.choiceItemClass + '">' +
						$('<div>').html(optionModel.text.charAt(0).toUpperCase() + optionModel.text.slice(1)).text() +
        ...
        setSelectedOption: function(option) {
            ...
            if (this.isMultiple) {
                ...
                this.$controls && this.addChoiceItem(option, 1);
    ...
    $.extend(OptionsCollection.prototype, {
        ...
        filter: function(query) {
            ...
            function checkItem(item) {
                return self.options.matcher(item.text.toLowerCase(), query.toLowerCase()) ? item : null;
            }
