		...
		*/	
		...
		$('#register_otp').unbind('keyup change').bind('keyup change', function (){
			...
			if(parseInt(otpval.length) == 8 && otpstat == 'valid'){
				...
				$.ajax({
					url: "<?=base_url();?>registration/get_regotp",
					type: "POST",
					data: {'action' : 'match_otp', 'otpval' : otpval, '<?=$csrf['name'];?>' : '<?=$csrf['hash'];?>'},
					success: function (data) {
						...
						if(jsona['oerror'] == 0 && jsona['oused'] == 0){
						    ...
						    if(jsona['oerror'] == 1 && jsona['oused'] == 0){
						        ...
						        $('.otp_error').html("Don't have Access code? Please <a href='<?=base_url();?>publicv/contact'>click here</a> to contact us.");
						    ...
						    if(jsona['oerror'] == 1 && jsona['oused'] == 1){
						        ...
						        $('.otp_error').html("Code was already used! Try another one or <a href='<?=base_url();?>publicv/contact'>click here</a> to contact us.");
    							setTimeout(function() {$('.otp_error').text('');}, 18000);
    							$('.reg_otp').find('.otp_sucess').hide();
    							$('#signupSubmit').attr('type', 'button');
    							$('#signupSubmit').attr('disabled', true);
		...
		$("#careers-form").validate({
			...
			messages: {
				...
				mfile: "Please upload Your resume(.doc,.docx,.pdf only)",
				mmob: {
				    required: "Please enter a valid mobile number"
				},
				defaultReal: "Please enter correct captcha (Letters are Case sensitive)."
		...
		$('.cat_select').bind('click', function(){
			...
			if(catid !== ''){
				$('<form id="search_form" action="<?=base_url();?>listing/search" method="post"><input type="hidden" name="col_name" value="'+colname+'" ><input type="hidden" name="col_val" value="'+catid+'" ><input type="hidden" name="action" value="search" /><input type="hidden" name="<?=$csrf['name'];?>" value="<?=$csrf['hash'];?>" /></form>').appendTo('body').submit();
			}
		});
		...
		$('.reactivate_account').bind('click', function(){
			...
			$('<form id="search_form" action="<?=base_url();?>login/reactivate_account" method="post"><input type="hidden" name="user_name" value="'+uemail+'" /><input type="hidden" name="action" value="reset_password" /><input type="hidden" name="<?=$csrf['name'];?>" value="<?=$csrf['hash'];?>" /></form>').appendTo('body').submit();
