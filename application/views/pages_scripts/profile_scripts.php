<script type="text/javascript">
	...
	$(function(){
		...
		jQuery.validator.addMethod("NEmailGeneral", function(value, element) {
			var re = /^([a-zA-Z])(.*[a-z])(.*[a-z0-9_\+-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*\.([a-z]{2,})$/;
			if (!value) {
				return true;
			 }else{
		...
		}, 'The text must start with a letter Accepts letter and numbers. Not accepts any special characters excepts[-]'); 
		...
		jQuery.validator.addMethod("linkedinURL", function(value, element) {
			...
			var urlFilter =  /(http|https):\/\/?(?:(www|\w\w)\.)?linkedin.com(\w+:{0,1}\w*@)?(\S+)(:([0-9])+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/;
		...
		$("#form_contractor_details").validate({
			...
			onkeyup: function(elem){
				...
				if(element_id == 'contract_amount' || element_id == 'cdeliv_date'){
					...
					var testr = /^<?=trim($uemail)?>+$/.test( value );
		...
		$('#xinfin_sign_in').on('hidden.bs.modal', function (e) {
			...
			$('<form action="<?=base_url();?>smartcontract/initiate" method="post"><input type="hidden" name="rproject_ref" value="'+proj_id+'" ><input type="hidden" name="ruser_id" value="'+user_id+'" ><input type="hidden" name="ruser_type" value="'+user_type_ref+'" ><input type="hidden" name="request_type" value="" ><input type="hidden" name="action" value="smart_contract" /><input type="hidden" name="<?=$csrf['name'];?>" value="<?=$csrf['hash'];?>" /></form>').appendTo('body').submit();
		});	
	...
	function click_handler(){
		...
		$('.del_services').unbind('click').bind('click', function(){
			...
			$.ajax({
				url: "<?=base_url();?>user/delete_service",
				type: "POST",
				data: {'serv_id' : serv_id, 'action' : 'delete_service', '<?=$csrf['name'];?>' : '<?=$csrf['hash'];?>'},
		...
		$('.del_products').unbind('click').bind('click', function(){
			...
			$.ajax({
				url: "<?=base_url();?>user/delete_product",
				type: "POST",
				data: {'prod_id' : prod_id, 'action' : 'delete_product', '<?=$csrf['name'];?>' : '<?=$csrf['hash'];?>'},
				success: function (data) {
		...
		$('.add_uproduct').unbind('click').bind('click', function(){
			...
			if($.trim(product_input_val) !== '' && parseInt(error) == 0){
				...
				$.ajax({
					url: "<?=base_url();?>user/check_product",
					type: "POST",
					data: {'product_input' : product_input_val, '<?=$csrf['name'];?>' : '<?=$csrf['hash'];?>'},
					success: function (data) {
						...
						if(parseInt(status_msg['error']) == 0){
							...
							$('.products_view').find('.left_li_text').append('<a href="javascript:void(0)">'+product_input_val.replace(/(^|\s)\S/g, l => l.toUpperCase())+' <span class="del_products" pid="'+status_msg['pid']+'"><i class="fa fa-times"></i></span></a>');
							$('#'+product_input).val('');
						}
		...
		$('.add_uservice').unbind('click').bind('click', function(){
			...
			if($.trim(service_input_val) !== '' && parseInt(error) == 0){
				...
				$.ajax({
					url: "<?=base_url();?>user/check_service",
					type: "POST",
					data: {'service_input' : service_input_val, '<?=$csrf['name'];?>' : '<?=$csrf['hash'];?>'},
					success: function (data) {
						...
						if(parseInt(status_msg['error']) == 0){
							...
							$('.services_view').find('.left_li_text').append('<a href="javascript:void(0)">'+service_input_val.replace(/(^|\s)\S/g, l => l.toUpperCase())+' <span class="del_services" sid="'+status_msg['sid']+'"><i class="fa fa-times"></i></span></a>');
							$('#'+service_input).val('');
		...
		/* $('.confirm_click').unbind('click').bind('click', function(){
			
			document.getElementById('fade').style.display = 'block';
			document.getElementById('page_wait_loader').style.display = 'block';
		}); */
		...
		$('.bproj_complete').bind('click', function(){
			...
			if(parseInt(proj_id) > 0 && parseInt(user_id) > 0 && parseInt(user_type_ref) > 0 && parseInt(ruser_id) > 0 && parseInt(ruser_type_ref) > 0){
				...
				$('<form id="search_form" action="<?=base_url();?>smartcontract/initiate" method="post"><input type="hidden" name="proj_id" value="'+proj_id+'" ><input type="hidden" name="user_id_request" value="'+ruser_id+'" ><input type="hidden" name="user_type_request" value="'+ruser_type_ref+'" ><input type="hidden" name="rproject_ref" value="'+proj_id+'" ><input type="hidden" name="ruser_id" value="'+ruser_id+'" ><input type="hidden" name="ruser_type" value="'+ruser_type_ref+'" ><input type="hidden" name="request_type" value="'+request_type+'" ><input type="hidden" name="request_db_type" value="initiate" ><input type="hidden" name="raction" value="request_completion" ><input type="hidden" name="action" value="smart_contract" /><input type="hidden" name="<?=$csrf['name'];?>" value="<?=$csrf['hash'];?>" /></form>').appendTo('body').submit();
			}
		...
		$('.initiate_finance').bind('click', function(){
			...
			$('<form id="search_form" action="<?=base_url();?>smartcontract/initiate" method="post"><input type="hidden" name="rproject_ref" value="'+fproj_ref+'" ><input type="hidden" name="rfusers" value="'+fusers+'" ><input type="hidden" name="ruser_id" value="0" ><input type="hidden" name="ruser_type" value="2" ><input type="hidden" name="request_type" value="" ><input type="hidden" name="raction" value="initiate_finance" ><input type="hidden" name="action" value="smart_contract" /><input type="hidden" name="<?=$csrf['name'];?>" value="<?=$csrf['hash'];?>" /></form>').appendTo('body').submit();
		...
		$('.pay_xinfin').unbind('click').bind('click', function(){
			...
			$.ajax({
				url: "<?=base_url();?>smartcontract/get_beneficiary_info",
				type: "POST",
				data: {'user_id' : puser_id, 'user_type' : puser_type, '<?=$csrf['name'];?>' : '<?=$csrf['hash'];?>'},
				success: function (data) {
			...
			if(parseInt(curr_user_type) == 2){
				...
				$.ajax({
					url: "<?=base_url();?>smartcontract/get_fproposal_info",
					type: "POST",
					data: {'user_id' : user_id, 'proj_id' : p_id, 'user_type' : user_type, '<?=$csrf['name'];?>' : '<?=$csrf['hash'];?>'},
					success: function (data) {
		...
		$('#update_wallet').unbind('click').bind('click', function(){
			...
			$.ajax({
				url: "<?=base_url();?>user/update_balance",
				type: "POST",
				data: {'action' : 'update_balance', '<?=$csrf['name'];?>' : '<?=$csrf['hash'];?>'},
				success: function (data) {
		...
		$('.repay_confirm_finacier').unbind('click').bind('click', function(){
			...
			$.ajax({
				url: "<?=base_url();?>smartcontract/confirm_beneficiary_payment",
				type: "POST",
				...
				success: function (data) {
					...
					if(parseInt(jsona['status']) == 1){
						...
						setTimeout(function(){ 
							...
							$('<form action="<?=base_url();?>smartcontract/initiate" method="post"><input type="hidden" name="rproject_ref" value="'+p_id+'" ><input type="hidden" name="ruser_id" value="'+puser_id+'" ><input type="hidden" name="ruser_type" value="'+puser_type+'" ><input type="hidden" name="request_type" value="" ><input type="hidden" name="action" value="smart_contract" /><input type="hidden" name="<?=$csrf['name'];?>" value="<?=$csrf['hash'];?>" /></form>').appendTo('body').submit();
...
// 			break;
		...
		$('#pay_financier').unbind('click').bind('click', function(){
			...
			$.ajax({
				url: "<?=base_url();?>smartcontract/pay_financier",
				type: "POST",
					...
					if(!$.isEmptyObject(jsona)){
						...
						if(parseInt(jsona['status']) == 1){
							...
							setTimeout(function(){ 
								...
								$('<form action="<?=base_url();?>smartcontract/initiate" method="post"><input type="hidden" name="rproject_ref" value="'+proj_id+'" ><input type="hidden" name="ruser_id" value="0" ><input type="hidden" name="ruser_type" value="2" ><input type="hidden" name="request_type" value="" ><input type="hidden" name="action" value="smart_contract" /><input type="hidden" name="<?=$csrf['name'];?>" value="<?=$csrf['hash'];?>" /></form>').appendTo('body').submit();
		...
		$('#pay_supplier').unbind('click').bind('click', function(){
			...
			if(parseInt(proj_id) > 0 && parseInt(user_id) > 0 && parseInt(user_type_ref) > 0 && parseInt(ruser_id) > 0 && parseInt(ruser_type_ref) > 0){
				...
				$.ajax({
					url: "<?=base_url();?>smartcontract/pay_supplier",
					type: "POST",
					data: {'proj_id' : proj_id, 'rproject_ref' : proj_id, 'user_id_request' : ruser_id, 'user_type_request' : ruser_type_ref, 'ruser_id' : ruser_id, 'ruser_type' : ruser_type_ref, 'request_type' : request_type, 'request_db_type' : 'payment', 'raction' : 'request_completion', 'trade_amtval' : trade_amtval, 'action' : 'smart_contract', '<?=$csrf['name'];?>' : '<?=$csrf['hash'];?>'},
					success: function (data) {
						...
						if(!$.isEmptyObject(jsona)){
							...
							if(parseInt(jsona['status']) == 1){
								...
								setTimeout(function(){ 
									...
									$('.modal_payment_close').trigger('click'); 
									$('<form action="<?=base_url();?>smartcontract/initiate" method="post"><input type="hidden" name="rproject_ref" value="'+proj_id+'" ><input type="hidden" name="ruser_id" value="'+ruser_id+'" ><input type="hidden" name="ruser_type" value="1" ><input type="hidden" name="request_type" value="" ><input type="hidden" name="action" value="smart_contract" /><input type="hidden" name="<?=$csrf['name'];?>" value="<?=$csrf['hash'];?>" /></form>').appendTo('body').submit();
		...
		$('#pay_beneficiary').unbind('click').bind('click', function(){
			...
			$.ajax({
				url: "<?=base_url();?>smartcontract/pay_beneficiary",
				type: "POST",
				...
				if($(this).hasClass('star')){
					...
					if(parseInt(score) > 0){
						...
						$.ajax({
							...
							data : {'project_ref': prow_id, 'rating_val': score, 'tuser_ref': to_user_id, 'tuser_type': to_user_type, 'action': 'add_rating', '<?=$csrf['name'];?>' : '<?=$csrf['hash'];?>'},
							success : function(data){
