				...
				<div class="container">
					<div class="common_dashboard_sec supp_finan_my_projects">
						...
						<div class="dynamic_table"> 
							<table id="accepted_listing_projects" class="table table-striped table-bordered" width="100%" cellspacing="0">
								<thead>
									<tr>
								...
								<tbody>
									<?php
										...
										if($projects_listed && !empty($projects_listed) && is_array($projects_listed) && sizeof($projects_listed) <> 0 && (!empty($proposal_accepted) || !empty($proposal_submitted) || !empty($proposals_subcontractor))){	
											...
											foreach($projects_listed as $plrow){
												...
												if(isset($proposal_details_financier[$plrow->ID]) && !empty($proposal_details_financier[$plrow->ID])){
									...
									<tr>
										<td>
											<a href="javascript:void(0)" puser="<?=$plrow->userID;?>" row_id="<?=$plrow->ID;?>" <?=(($user_id <> 0) ? 'class="proj_info more_info"' : '');?>><?='TF-'.strtotime($plrow->postDate);?></a>
										</td>
										<td>
											<?php  
										...
										?>		
												<a class="proj_info view_propose_btn tooltipa" user_type_ref="<?=$user_type_ref;?>" user_id="<?=$user_id;?>" row_id="<?=$plrow->ID;?>"><i class="fa fa-eye"></i> <span class="tooltipatext">View Proposal</span></a>&nbsp;<a class="send_message tooltipa send_message_btn" proj_id="<?=$plrow->ID;?>" send_user="<?=((!empty($supplier_info) && is_array($supplier_info) && sizeof($supplier_info) <> 0) ? $supplier_info[$plrow->ID][0] : $plrow->userID);?>" send_user_type="<?=((!empty($supplier_info) && is_array($supplier_info) && sizeof($supplier_info) <> 0) ? 1 : $plrow->userType);?>"><span><i class="fa fa-comments"></i></span> <span class="tooltipatext">Send Message</span></a>
										<?php		
											}else{	
										?>
											<a class="view_propose view_propose_btn tooltipa" user_type_ref="<?=$user_type_ref;?>" user_id="<?=$user_id;?>" row_id="<?=$plrow->ID;?>" prow_id="<?=(isset($proposal_info[$plrow->ID][0]) ? $proposal_info[$plrow->ID][0] : 0);?>"><i class="fa fa-eye"></i> <span class="tooltipatext">View Proposal</span></a>&nbsp;<a class="send_message send_message_btn tooltipa" proj_id="<?=$plrow->ID;?>" send_user="<?=((!empty($supplier_info) && is_array($supplier_info) && sizeof($supplier_info) <> 0) ? $supplier_info[$plrow->ID][0] : $plrow->userID);?>" send_user_type="<?=((!empty($supplier_info) && is_array($supplier_info) && sizeof($supplier_info) <> 0) ? 1 : $plrow->userType);?>" style=""><span><i class="fa fa-comments"></i></span> <span class="tooltipatext">Send Message</span></a>
										<?php 	}
											}
										?>
										</td>
					...
					<div class="common_dashboard_sec supp_finan_my_invitations">
						...
						<div class="dynamic_table"> 
							<table id="invited_listing_projects" class="table table-striped table-bordered" width="100%" cellspacing="0">
								<thead>
									<tr>
										<th style="width:11%;">Project No</th>
										<th style="width:16%;">Project Title</th>
								...
								<tbody>
									...
									?>
									<tr>
										<td><a href="javascript:void(0)" puser="<?=$ai_row->userID;?>" row_id="<?=$ai_row->ID;?>" <?=(($user_id <> 0) ? 'class="proj_info more_info"' : '');?>><?='TF-'.strtotime($ai_row->postDate);?></a></td>
										<td>
											<?php  
					...
					<div class="common_dashboard_sec supp_finan_my_saved_projects">
						...
						<div class="dynamic_table"> 
							<table id="saved_listing_projects" class="table table-striped table-bordered" width="100%" cellspacing="0">
								<thead>
									<tr>
										<th style="width:11%;">Project No</th>
										<th style="width:16%;">Project Title</th>
								...
								<tbody>
									...
									<tr>
										...
										<td class="text-center sp_action">
											...
											<a class="send_query send_message_btn tooltipa" proj_id="'.$plrow->ID.'" send_user="'.$plrow->userID.'" send_user_type="'.$plrow->userType.'"><i class="fa fa-comments"></i> <span class="tooltipatext">Send Message</span></a>&nbsp;
				...
				<div class="container">
					<div class="common_dashboard_sec beneficiary_my_projects">
						<div class="dsahboard_header_sec">
							...
							<?php if($user_type_ref == 1 || $user_type_ref == 2){  }else{ ?>
							<a class="dash_browse_btn" href="<?=base_url()?>listing/create_project"> Create Projects</a>
							<?php } ?>
						</div>
						<div class="dynamic_table"> 
							
							<table id="projects_table" class="table table-striped table-bordered" width="100%" cellspacing="0">
								<thead>
									<tr>
								...
								<tbody>
									...
									<tr>
										<td class="pro_no" style="width:11%;">
											<a href="javascript:void(0)" puser="<?=$plrow->userID; ?>" row_id="<?=$plrow->ID; ?>" <?=(($user_id <> 0) ? 'class="proj_info"' : '')?>><?php echo 'TF-'.strtotime($plrow->postDate) ?></a>
										</td>
										<td class="col-md-3">
											<?php  
