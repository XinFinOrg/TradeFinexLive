...
(d=f.buttonLiner);if(a.available&&!a.available(g,a))return!1;buttonc = 'fa-'+this.c.buttons[count].extend; buttonp = this.c.buttons[count].extend == 'pdf' ? 'fa-file-pdf-o' : ''; buttonf = this.c.buttons[count].extend == 'csv' ? 'fa-file' : ''; buttone = this.c.buttons[count++].extend == 'excel' ? 'fa-file-excel-o' : ''; buttonc = buttonp != '' ? buttonp : buttonc; buttonc = buttonf != '' ? buttonf : buttonc;buttonc = buttone != '' ? buttone : buttonc;console.log(this.c);var i=e("<"+c.tag+"/>").addClass(c.className).attr("tabindex",this.s.dt.settings()[0].iTabIndex).append("<i class='fa "+buttonc+"'></i> ").attr("aria-controls",this.s.dt.table().node().id).on("click.dtb",function(b){b.preventDefault();!i.hasClass(c.disabled)&&a.action&&a.action.call(g.button(i),b,g,i,a);i.blur()}).on("keyup.dtb",function(b){b.keyCode===13&&!i.hasClass(c.disabled)&&a.action&&a.action.call(g.button(i),b,g,i,a)});d.tag?i.append(e("<"+d.tag+"/>").html(h(a.text)).addClass(d.className)):
