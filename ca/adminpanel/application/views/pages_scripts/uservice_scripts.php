<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$('input[name="uservapprove"]').bootstrapSwitch({
			 size: 'xs',
			 on: 'Y',
			 off: 'N'
		});
						
		$('input[name="uservreject"]').bootstrapSwitch({
			 size: 'xs',
			 on: 'Y',
			 off: 'N'
		});
	...
	function click_handler(){
		...
		$('input[name="uservapprove"]').change(function(e) {
			
			var srow_id = $(this).attr('usid');
			...
			if($(this).attr('disabled') == 'disabled'){
				...
				if(!confirm('Do you want to do this ?')) {
					...
					$.ajax({
						...
						type : 'POST',
						url : '<?=base_url();?>users/service_admin_approve',
						data : {'service_ref': srow_id, 'approve_val': 1, 'category_option': coption, 'mail_body' : ''},
		...
		$('input[name="uservreject"]').change(function(e) {
		...
		$('.send_smail').bind('click', function(){
			
			var srow_id = $('#mrow_id').val();
			...
			if($.trim(reason_msg) == ''){
				...
				$.ajax({
					...
					type : 'POST',
					url : '<?=base_url();?>users/service_admin_approve',
					data : {'service_ref': srow_id, 'approve_val': 2, 'category_option': coption, 'mail_body' : reason_msg},
					success : function(data){
						// console.log('success'+data);
						window.location = '<?=base_url();?>users/service_lists_rejected';
						$('#mrow_id').val(0);
						$('#reason_msg').val('');
						click_handler();
					},
