<script type="text/javascript">
	...
	$(function(){
		...
		$("#form_post_project").validate({
			...
			messages: {
				...
				refnum: {
					required: "Please choose a refferrence number",
					minlength: "Refference number should be at least 4 characters long"
				}
			},
		...
		<?php if(!empty($_POST) && isset($_POST['action']) && $_POST['action'] == 'edit'){ }else{ ?>		
		...
		?>
			...
			setTimeout( function(){ $('<form id="search_form" action="<?=base_url();?>listing/project_info" method="post"><input type="hidden" name="row_id" value="<?php echo $proj_row ?>" ><input type="hidden" name="action" value="project_info" /><input type="hidden" name="<?=$csrf['name'];?>" value="<?=$csrf['hash'];?>" /></form>').appendTo('body').submit(); }, 3000 ); 
		...
		?>		
					setTimeout( function(){ $('<form id="search_form" action="<?=base_url();?>smartcontract/initiate" method="post"><input type="hidden" name="rproject_ref" value="<?php echo $_POST['row_id'] ?>" ><input type="hidden" name="ruser_id" value="<?php echo $_POST['user_id'] ?>" ><input type="hidden" name="ruser_type" value="<?php echo $_POST['user_type'] ?>" ><input type="hidden" name="request_type" value="complete_supplier" ><input type="hidden" name="action" value="smart_contract" /><input type="hidden" name="<?=$csrf['name'];?>" value="<?=$csrf['hash'];?>" /></form>').appendTo('body').submit(); }, 3000 );
		...
		?>		
					setTimeout( function(){ $('<form id="search_form" action="<?=base_url();?>listing/project_info" method="post"><input type="hidden" name="row_id" value="<?php echo $_POST['row_id'] ?>" ><input type="hidden" name="action" value="project_info" /><input type="hidden" name="<?=$csrf['name'];?>" value="<?=$csrf['hash'];?>" /></form>').appendTo('body').submit(); }, 3000 );
		...
		$('.search_project_post').bind('click', function(){
			...
			$('<form id="search_form" action="<?=base_url();?>listing/search" method="post"><input type="hidden" name="col_name" value="'+colname+'" ><input type="hidden" name="col_val" value="'+strc+'" ><input type="hidden" name="action" value="search" /><input type="hidden" name="<?=$csrf['name'];?>" value="<?=$csrf['hash'];?>" /></form>').appendTo('body').submit();
		});
				
		$('.group_head').bind('click', function(){
		...
		$('.database_op').bind('click', function(){
			...
			$('<form id="search_form" action="<?php echo $_SERVER['REQUEST_URI'] ?>" method="post"><input type="hidden" name="sort_order" value="'+sval+'" ><input type="hidden" name="action" value="search" /><input type="hidden" name="<?=$csrf['name'];?>" value="<?=$csrf['hash'];?>" /></form>').appendTo('body').submit();
		});
		...
		$('.proposal_accept').bind('click', function(){
		...
		$('.proposal_reject').bind('click', function(){
		...
		var rater = $('.star').raty({
			...
			click:  function(score, evt) { 
				...
				var integer = score.toString().split('.')[0];
				var decimal = score.toString().split('.')[1];
				var scorev = 0;
				
				if(parseInt(decimal) >= 10 && parseInt(decimal) <= 50){
					decimal = 5;
					scorev = integer+'.'+decimal;
				}
				
				if(parseInt(decimal) > 50){
					scorev = parseInt(integer) + 1;
				}
				...
				if(parseInt(scorev) > 0){
				
					$.ajax({
											
						type : 'POST',
						url : '<?=base_url();?>project/add_rating_project_user',
						data : {'project_ref': prow_id, 'rating_val': scorev, 'tuser_ref': to_user_id, 'tuser_type': to_user_type, 'action': 'add_rating', '<?=$csrf['name'];?>' : '<?=$csrf['hash'];?>'},
						success : function(data){
							console.log(data);
							click_handler();
						}		
	...
	function click_handler(){
		...
		$('.pcompletion_accept').bind('click', function(){
		...
		$('.pcompletion_reject').bind('click', function(){
		...
		$('.rproj_initiate').bind('click', function(){
