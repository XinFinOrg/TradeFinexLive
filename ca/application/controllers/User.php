...
class User extends CI_Controller {
	...
	public function add_product_service(){
		...
		$data['page'] = 'User Add Product Service';
		$data['msg'] = '';
		$data['msg_extra'] = '';
		$data['full_name'] = '';
		$data['user_id'] = 0;
		...
		if($action == 'add_product' && $data['user_id'] <> 0){
			
			$data_add = array();
					
			$data_add = array();
			$data_add['tfup_name'] = $this->input->post('uprodn');
			$data_add['tfup_admin_approval'] = 0;
			$data_add['tfup_updated'] = date('Y-m-d H:i:s');
			...
			if(!empty($upresult) && is_array($upresult) && sizeof($upresult) <> 0){
				...
				$data['msg'] = 'success';
				$data['msg_extra'] = 'Product has been added successfully.';
			}else{
				$data['msg'] = 'error';
				$data['msg_extra'] = 'Error occurred during product addition.';
			}
		}
		
		if($action == 'update_product' && $data['user_id'] <> 0){
			...
			$upresult = $this->manage->update_user_product_by_id($row_id, $data_add);
				
			if(!empty($upresult) && is_array($upresult) && sizeof($upresult) <> 0){
			
				$data['msg'] = 'success';
				$data['msg_extra'] = 'Product has been updated successfully.';
			}
			else{
				$data['msg'] = 'error';
				$data['msg_extra'] = 'Error occurred during product update.';
			}
		}
	...
	public function add_product(){
		...
		if($action == 'add_product' && $data['user_id'] <> 0){
			...
			$data_add = array();
			$config['upload_path']          = FCPATH.'assets/user_products_image/';
	...
	public function add_service(){
		...
		if($action == 'add_service' && $data['user_id'] <> 0){
						
			$usservcat = $this->input->post('usservcat');
			
			$data_add['tfuct_name'] = $this->input->post('uservcat');
			$data_add['tfuct_type'] = 2;
			$data_add['tfuct_user_ref'] = $data['user_id'];
			$data_add['tfuct_user_type_ref'] = $data['user_type_ref'];
			$data_add['tfuct_admin_approval'] = 0;
			$data_add['tfuct_updated'] = date('Y-m-d H:i:s');
			$ucid = 0;
				
			if($usservcat > 0){
				$this->manage->update_user_category_by_id($usservcat, $data_add);
				$ucresult = $this->manage->get_user_category_by_id($usservcat);
				$ucid = $ucresult[0]->tfuc_id;
			}else{
				$ucresult = $this->manage->add_user_category_temp($data['user_id'], $data['user_type_ref'], $data_add);
				$ucid = $ucresult[0]->tfuct_id;
			}
					
			if(!empty($ucresult) && is_array($ucresult) && sizeof($ucresult) <> 0){
				
				$data_add = array();
				$data_add['tfus_name'] = $this->input->post('uservn');
				$data_add['tfus_description'] = $this->input->post('uservdesc');
				$data_add['tfus_category_ref'] = $ucid;
				$data_add['tfus_user_ref'] = $data['user_id'];
				$data_add['tfus_user_type_ref'] = $data['user_type_ref'];
				$data_add['tfus_admin_approval'] = 0;
				$data_add['tfus_updated'] = date('Y-m-d H:i:s');
				...
				if(!empty($usresult) && is_array($usresult) && sizeof($usresult) <> 0){
					...
					$data['msg'] = 'success';
					$data['msg_extra'] = 'Service has been added successfully.';
					
				}else{
					$data['msg'] = 'error';
					$data['msg_extra'] = 'Only service category added.';
				}	
				
			}else{
				$data['msg'] = 'error';
				$data['msg_extra'] = 'Error occurred during service addition.';
			}
		}
		
		if($action == 'update_service' && $data['user_id'] <> 0){
			...
			if(!empty($ucresult) && is_array($ucresult) && sizeof($ucresult) <> 0){
				...
				$usresult = $this->manage->update_user_service_by_id($row_id, $data_add);
				
				if(!empty($usresult) && is_array($usresult) && sizeof($usresult) <> 0){
			
					$data['msg'] = 'success';
					$data['msg_extra'] = 'Service has been updated successfully.';
					
				}else{
					$data['msg'] = 'error';
					$data['msg_extra'] = 'Only service category updated.';
				}	
				
			}else{
				$data['msg'] = 'error';
				$data['msg_extra'] = 'Error occurred during service update.';
			}
		}
	...
	public function get_all_products(){
		...
		$data = $this->manage->get_user_product_public();
		
		if(!empty($data) && sizeof($data) <> 0){
			$i = 0;
			foreach($data as $row){
				// $result[$i]['text'] = ucwords($row->tfup_name);
				// $result[$i++]['value'] = ucwords($row->tfup_name);
				if($i == 0){
					$string = '{"text": "'.ucwords($row->tfup_name).'", "value": "'.ucwords($row->tfup_name).'"}';
				}else{
					$string .= ','.'{"text": "'.ucwords($row->tfup_name).'", "value": "'.ucwords($row->tfup_name).'"}';
				}
