...
class Project extends CI_Controller {
    ...
    public function proposal_supplier() {
        ...
        if ($action == 'update_proposal' && $prow_id <> 0) {
            ...
            $data_add['tpp_completion_time_value'] = $this->input->post('pcommission');
            $data_add['tpp_completion_time_ref'] = $this->input->post('pcommissionu');
            $data_add['tpp_beneficiary_edit_mode'] = 0;
            ...
            if ($result && !empty($result) && $result > 0) {
                ...
                for ($i = 0; $i < $filesCount; $i++) {
                    ...
                    if (is_array($uploadData) && !empty($uploadData) && !empty($uploadData[$i])) {
                        ...
                        $data_add_file = array();
                        $data_add_file['tpfsf_proposal_ref'] = $result;
                        $data_add_file['tpfsf_row_deleted'] = 0;
                        $data_add_file['tpfsf_file_index'] = ($i + 1);
                        $file_namea = explode('.', $uploadData[$i]['file_name']);

                        $file_row = $this->plisting->add_proposal_file_by_id($result, $data_add_file, 'f');

                        $data_add_file = array();
                        $data_add_file['tpfsf_filename'] = $file_row . '_' . $result . '_proposal_financier_' . ($i + 1) . '.' . end($file_namea);

                        $this->plisting->update_proposal_file_by_id($file_row, $data_add_file, 'f');

                        rename(FCPATH . 'assets/project_proposals/' . $uploadData[$i]['file_name'], FCPATH . 'assets/project_proposals/' . $file_row . '_' . $result . '_proposal_financier_' . ($i + 1) . '.' . end($file_namea));
                    }
                }
                $success_data = $this->upload->data();
                $data['msg'] = 'success';
                ...
                for ($i = 0; $i < $filesCount; $i++) {
                    ...
                    if (is_array($uploadData) && !empty($uploadData) && !empty($uploadData[$i])) {
                        ...
                        if ($uploadData[$i]['file_name'] != '') {
                            ...
                            if (trim($pdoc_id[$i]) == '' || $pdoc_id[$i] == 0) {
                                $data_add_file['tpfsf_proposal_ref'] = $result[0]->tpf_id;
                                $data_add_file['tpfsf_file_index'] = ($i + 1);

                                $presult = $this->plisting->get_proposal_file_by_ref_and_index($row_id, $data_add_file['tpfsf_file_index'], 'f');

                                if (!empty($presult) && is_array($presult) && sizeof($presult) <> 0) {
                                    $this->plisting->update_proposal_file_by_id($presult[0]->tpfsf_id, $data_add_file, 'f');
                                    $pdoc_id[$i] = $presult[0]->tpssf_id;
                                } else {
                                    $pdoc_id[$i] = $file_row = $this->plisting->add_proposal_file_by_id($row_id, $data_add_file, 'f');
                                }
                            } else {
                                $file_row = $this->plisting->update_proposal_file_by_id($pdoc_id[$i], $data_add_file, 'f');
                            }

                            $data_add_file = array();
                            $data_add_file['tpfsf_filename'] = $pdoc_id[$i] . '_' . $row_id . '_proposal_financier_' . ($i + 1) . '.' . end($file_namea);

                            $this->plisting->update_proposal_file_by_id($pdoc_id[$i], $data_add_file, 'f');

                            rename(FCPATH . 'assets/project_proposals/' . $uploadData[$i]['file_name'], FCPATH . 'assets/project_proposals/' . $pdoc_id[$i] . '_' . $row_id . '_proposal_financier_' . ($i + 1) . '.' . end($file_namea));
                        }
                    }
                }
    ...
    public function proposalv() {
        ...
        if ($action == 'proposal_accept' && $prow_id <> 0) {
            ...
            if ($ruser_type == 1) {
                ...
                if (!empty($result) && is_array($result) && sizeof($result) <> 0) {
                    ...
                    $result1 =  $this->plisting->update_project_by_id($row_id, $data_add);

                    if (!empty($result1) && is_array($result1) && sizeof($result1) <> 0) {
        ...
        if (($action == 'request_message_to_modify' || $action == 'proposal_accept' || $action == 'proposal_reject' || $action == 'proposal_view') && $prow_id <> 0) {
            ...
            if ($ruser_type == 2) {
                ...
                $presult = $this->plisting->get_proposal_by_id($prow_id, 'f');
                 $data['proposal_files'] = $this->plisting->get_proposal_files_by_id($prow_id, 'f');
                 
//                 print_r( $data['proposal_files']);
