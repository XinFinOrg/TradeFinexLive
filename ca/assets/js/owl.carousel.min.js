...
this._controls.$indicators=options.dotsContainer?$(options.dotsContainer):$('<div>').hide().addClass(options.dotsClass).appendTo(this._controls.$container);this._controls.$indicators.on('click','div',$.proxy(function(e){var index=$(e.target).parent().is(this._controls.$indicators)?$(e.target).index():$(e.target).parent().index();e.preventDefault();this.to(index,options.dotsSpeed)},this));$container=options.navContainer?$(options.navContainer):$('<div>').addClass(options.navContainerClass).prependTo(this._controls.$container);this._controls.$next=$('<'+options.navElement+'></div>');this._controls.$previous=this._controls.$next.clone();var blur_container_left=$('<div class="blur-effect-left"></div>').prependTo($container);var blur_container_right=$('<div class="blur-effect-right"></div>').appendTo($container);this._controls.$previous.addClass(options.navClass[0]).html(options.navText[0]).hide().insertAfter(blur_container_left).on('click',$.proxy(function(e){this.prev(options.navSpeed)},this));this._controls.$next.addClass(options.navClass[1]).html(options.navText[1]).hide().insertAfter(blur_container_right).on('click',$.proxy(function(e){this.next(options.navSpeed)},this));for(override in this._overrides){this._core[override]=$.proxy(this[override],this)}}
